...
public class SlackNewEventSender extends SlackEventSender {
	private SlackNewEventSender(SlackInput input, ContextArgs contextArgs, Model model, EventTokenizer tokenizer,
			EventResult event) {
	...
	public static SlackSender create(SlackInput input, ContextArgs contextArgs) {
		Model model = Models.newEvent();

		if (model == null) {
			// Logging happens inside newEvent.
			//
			return null;
		}
		...
		return new SlackNewEventSender(input, contextArgs, model, EventTokenizer.from(contextArgs, event), event);
	}
}
