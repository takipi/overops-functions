...
{
	public static void send(ApiClient apiClient, String serviceId, String viewId, Collection<EventResult> events,
			DateTime from, DateTime to, String desc)
	{
		...
		AnomalyAlertRequest anomalyAlertRequest = AnomalyAlertRequest.newBuilder().setServiceId(serviceId)
				.setViewId(viewId).setFrom(from.toString()).setTo(to.toString()).setDesc(desc)
