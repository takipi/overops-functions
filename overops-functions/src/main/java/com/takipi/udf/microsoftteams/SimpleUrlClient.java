...
public class SimpleUrlClient extends UrlClient {
	SimpleUrlClient(String hostname, int connectTimeout, int readTimeout, LogLevel defaultLogLevel,
	...
	public <T extends ApiResult> Response<String> post(ApiPostRequest<T> request) {
		try {
			...
			return post(request.urlPath(), data, request.contentType(), request.queryParams());
		} catch (Exception e) {
