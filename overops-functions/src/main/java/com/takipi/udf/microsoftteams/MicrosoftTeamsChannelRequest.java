...
public class MicrosoftTeamsChannelRequest implements ApiPostRequest<EmptyResult> {
	...
	public MicrosoftTeamsChannelRequest(String url, String exceptionLinkToOverOps, String exceptionClassName,
			String exceptionLocationPath, List<Location> stack_frames, String deployment, String application,
			String server, String environmentsName, String reportedBy, String doNotAlertLink) {
		this.url = url;
	...
	public String postData() {
		MicrosoftCard microsoftCard = MicrosoftCard.newBuilder().setThemeColor(THEME_COLOR)
				.setText(new MicrosoftTextBuilder().add("A new ").addBold(exceptionClassName).add(" in ")
						.addBold(exceptionLocationPath).add(" has been detected in ").addBold(environmentsName)
						.add(StringUtils.isNotEmpty(reportedBy) ? new MicrosoftTextBuilder().add(" (alert added by ")
								.addBold(reportedBy).add(")").build() : "")
						.build())
				.addSections(new MicrosoftTextSection(new MicrosoftTextBuilder()
						.addLink(exceptionLinkToOverOps, exceptionClassName).addEnter()
						.addHighlighted(new MicrosoftTextBuilder().addArray(stack_frames, " at ").build()).build()),
						new MicrosoftActivitySection.Builder().addFacts(new MicrosoftFact("Server", server),
								new MicrosoftFact("Application", application),
								new MicrosoftFact("Deployment", deployment)).build())
				.addPotentialActions(
						MicrosoftPotentialAction.newBuilder().setName("View Event")
								.addTargets(new MicrosoftTarget(exceptionLinkToOverOps)).build(),
						MicrosoftPotentialAction.newBuilder().setName("Do not alert on new " + exceptionClassName)
								.addTargets(new MicrosoftTarget(doNotAlertLink)).build())
				.build();
	...
	public static class Builder {
		...
		public MicrosoftTeamsChannelRequest build() {
			return new MicrosoftTeamsChannelRequest(url, exceptionLink, exceptionClassName, exceptionLocationPath,
					stack_frames, deployment, application, server, environmentsName, reportedBy, doNotAlertLink);
		}
	}
}
